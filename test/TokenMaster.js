...
const OCCASION_NAME = "ETH London"
const OCCASION_COST = ethers.utils.parseUnits('1','ether')
const OCCASION_MAX_TICKETS = 100
const OCCASION_DATE = "Apr 27"
const OCCASION_TIME = "10:00AM GMT+1"
const OCCASION_LOCATION = "England , London"
...
describe("TokenMaster", () => {
    ...
    describe("Occasions", ()=>{
        it("Updates the occasions count" , async()=>{
            const totalOccaions = await tokenMaster.totalOccasion();
            expect(totalOccaions).to.be.equal(1);
        })
    ...
    describe("Minting" , ()=>{
        ...
        beforeEach(async()=>{
            const transaction = await tokenMaster.connect(buyer).mint(
                ID,
                SEAT,
                {value:AMOUNT}
            )
        ...
        it("Updates the token owner",async()=>{
            const ticketOwner = await tokenMaster.seatTaken(ID , SEAT)
            expect(ticketOwner).to.be.equal(buyer.address)
        })
        it("Updates the total supply count",async()=>{
